...
for n_neighbors = 20
    for M = 10
        ...
        nl = build_graph_sym_lap(n_neighbors,M,distance_matrix,num_ver);
        [eigVectors,eigValues] = eig(nl);
        eig_filename = ['C:\Users\DELL\Desktop\ntu2000experiment\n_',num2str(n_neighbors),'_M_',num2str(M),'.mat'];
        save(eig_filename,'eigVectors','eigValues');
        
        % multiclass segmentation
        disp(['--n_neighbors:',num2str(n_neighbors),' --M:',num2str(M)]);
        for mu = [10] %100 200 400 500] %200:50:500 600:100:900 1000:200:5000]
            for dt = [1] %[0.0005 0.001 0.005 0.01 0.05 0.1 0.5 1 ]%0.01:0.02:0.09 0.1:0.1:1 1:1:10]
                for Ns = 1:1:3
                    for fid_num = 1:10
